...
def get_arguments():
    ...
    try:
        ...
        return sys.argv[1], process_filename(sys.argv[2])
...
def get_villages(url: str) -> dict():
    try:
        ...
        def add_village(number, name, link):
            if link is None:
                return
        ...
        for tr in rows:
            ...
            if len(columns) >= 3:
                ...
                if len(columns) == 3 or len(columns) == 7:
                    add_village(columns[0].text, columns[1].text, columns[0].find('a'))
                    if len(columns) == 7:
                        add_village(columns[4].text, columns[5].text, columns[4].find('a'))
                elif len(columns) == 4:
    ...
    def process_header_table(rows):
        ...
        for row in rows:
            ...
            if columns is not None and len(columns) >= 7:
                csv_row = [columns[3].text, columns[4].text, columns[7].text]
                break
        return csv_row
